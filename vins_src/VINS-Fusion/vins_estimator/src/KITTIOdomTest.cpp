...
int main(int argc, char** argv)
{
    ...
    if (argc != 3)
    {
        printf("please input: rosrun vins kitti_odom_test [config file] [data folder] \n"
    ...
    FILE* outFile;
    outFile = fopen((OUTPUT_FOLDER + "/vio_kitti.txt").c_str(), "w");
    if (outFile == NULL)
    ...
    for (size_t i = 0; i < imageTimeList.size(); i++)
    {
        if (ros::ok())
        {
            ...
            leftImagePath = dataPath + "image_0/data/" + ss.str() + ".png";
            rightImagePath = dataPath + "image_1/data/" + ss.str() + ".png";
            printf("leftImagePath: %s\n", leftImagePath.c_str());

            imLeft = cv::imread(leftImagePath, cv::IMREAD_GRAYSCALE); // Updated to cv::IMREAD_GRAYSCALE
            ...
            imRight = cv::imread(rightImagePath, cv::IMREAD_GRAYSCALE); // Updated to cv::IMREAD_GRAYSCALE
